    ...
    try {
      ...
      if (response.status === 200) {
        ...
        setData({
          ...
          firebaseImageUrl: propertyData.firebaseImageUrl,
        });
  ...
  const handleUpdate = async (e) => {
    ...
    if (newImage) {
      try {
        ...
        const imgUpload = await axios.post(`${baseURL}/upload`, imageData);
  
        if (imgUpload.status === 200) {
          const imageUrl = imgUpload.data.imageUrl;
          updatedData = { ...updatedData, firebaseImageUrl: imageUrl };  
          setData(updatedData); 
        } else {
          toast.error("Error uploading image to Firebase");
        }
      } catch (err) {
        console.error(err);
        toast.error("Error uploading image to Firebase");
    ...
    try {
      const updateResponse = await axios.put(`${baseURL}/update/${id}`, updatedData);  
